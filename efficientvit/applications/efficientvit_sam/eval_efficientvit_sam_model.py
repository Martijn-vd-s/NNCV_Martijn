...
def predict_mask_from_box(
    predictor: EfficientViTSamPredictor, bbox: np.ndarray
) -> np.ndarray:
...
class eval_dataset(Dataset):
    def __init__(
        ...
        if self.dataset == "coco":
            ...
            self.images = [
                os.path.join(self.image_root, image) for image in self.images
            ]
            self.ids = [
                int(image.split("/")[-1].split(".")[0]) for image in self.images
            ]
        ...
        if self.prompt_type == "point" or self.prompt_type == "box":
            self.annotations = json.load(open(self.annotation_json_file, "r"))[
                "annotations"
            ]
    ...
    def __getitem__(self, idx):
        ...
        if self.prompt_type == "point" or self.prompt_type == "box":
            ...
            detections = [
                det for det in self.source_json_file if det["image_id"] == self.ids[idx]
            ]
    ...
    for _, data in enumerate(tqdm(dataloader, disable=local_rank != 0)):
        ...
        for ann in anns:
            ...
            for i in range(num_click):
                ...
                pre_mask = predict_mask_from_point(
    ...
    for _, data in enumerate(tqdm(dataloader, disable=local_rank != 0)):
        ...
        for det in detections:
            ...
            rle = mask_util.encode(
                np.array(sam_mask[:, :, None], order="F", dtype="uint8")
            )[0]
...
def evaluate(results, prompt_type, dataset, annotation_json_file=None):
    if prompt_type == "point" or prompt_type == "box":
        print(
            ", ".join(
                [f"{key}={val:.3f}" for key, val in get_iou_metric(results).items()]
            )
        )
        ...
        if dataset == "coco":
            ...
            evaluate_predictions_on_coco(
                coco_gt=coco_api, coco_results=results, iou_type=iou_type
            )
        elif dataset == "lvis":
            lvis_api = LVIS(annotation_json_file)
            evaluate_predictions_on_lvis(
                lvis_gt=lvis_api, lvis_results=results, iou_type=iou_type
            )
    else:
        raise NotImplementedError()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="point",
        choices=["point", "box", "box_from_detector"],
    )
    ...
    dataset = eval_dataset(
        ...
        args.image_root,
        args.prompt_type,
        args.annotation_json_file,
        args.source_json_file,
    )
